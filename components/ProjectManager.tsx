...
const ProjectManager: React.FC<ProjectManagerProps> = ({ isOpen, onClose, projects, onAddProject, onRenameProject, onDeleteProject }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Project Manager">
        <div className="space-y-4">
            {/* Add Project Form */}
            <form onSubmit={handleAddProject} className="space-y-2 pb-4 border-b border-jam-border dark:border-slate-700">
                <label htmlFor="new-project-name" className="block text-sm font-medium text-slate-600 dark:text-slate-300">Create a New Project</label>
                <div className="flex space-x-2">
                    <input
                        ...
                        className="flex-grow bg-white dark:bg-slate-900 border border-jam-border dark:border-slate-600 rounded-md shadow-sm py-2 px-3 text-jam-dark dark:text-slate-200 focus:outline-none focus:ring-jam-blue dark:focus:ring-pink-500 sm:text-sm"
                        placeholder="e.g., Final Exams Sprint"
                    />
                    <button type="submit" className="px-4 py-2 text-sm font-semibold text-white bg-jam-dark rounded-lg hover:bg-black dark:bg-pink-600 dark:hover:bg-pink-700 disabled:bg-slate-400 dark:disabled:bg-slate-600" disabled={!newProjectName.trim()}>
                        Add
                    </button>
                </div>
            </form>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                <h3 className="text-sm font-medium text-slate-600 dark:text-slate-300">Existing Projects</h3>
