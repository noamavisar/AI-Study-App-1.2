...
import { LearningResource, ResourceType, ProjectFile } from '../types';
...
const LearningResourcesModal: React.FC<LearningResourcesModalProps> = ({ isOpen, onClose, onGenerate, isLoading, projectFiles }) => {
  const [tempResources, setTempResources] = useState<LearningResource[]>([]);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
        const newResources: LearningResource[] = files.map(file => ({
            ...
            file,
            type: ResourceType.LearningMaterial,
        }));
        setTempResources(prev => {
            const existingIds = new Set(prev.map(r => r.id));
            const trulyNew = newResources.filter(nr => !existingIds.has(nr.id));
  ...
  const updateTempResourceType = (id: string, type: ResourceType) => {
    setTempResources(prev => prev.map(r => r.id === id ? { ...r, type } : r));
  };

  const removeTempResource = (id: string) => {
    setTempResources(prev => prev.filter(r => r.id !== id));
  };
  ...
  const handleGenerate = () => {
    const uploadedResources = tempResources;
    ...
    const projectResources: Omit<LearningResource, 'id'>[] = projectFilesAsFiles.map(file => ({
        file,
        type: ResourceType.LearningMaterial, // Default type for project files
    }));

    const allResources = [...uploadedResources, ...projectResources];

    if (allResources.length === 0) {
      setError('Please upload or select at least one learning resource.');
      return;
    }
    if (days < 1) {
  ...
  const totalSelectedCount = tempResources.length + selectedProjectFileIds.size;

  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Generate Study Sprint with AI">
      <div className="space-y-4">
        <p className="text-sm text-slate-600 dark:text-slate-300">
          Provide your course materials and the AI will create a personalized study plan for you.
        </p>
        ...
        <div>
            <label className="block text-sm font-medium text-slate-600 dark:text-slate-300 mb-2">Learning Resources ({totalSelectedCount} selected)</label>
            ...
            {activeTab === 'upload' && (
                <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-jam-border dark:border-slate-600 border-dashed rounded-md">
                    <div className="space-y-1 text-center">
                        ...
                        <div className="flex text-sm text-slate-600 dark:text-slate-400"><label htmlFor="file-upload" className="relative cursor-pointer bg-white dark:bg-slate-800 rounded-md font-medium text-jam-blue dark:text-pink-500 hover:text-jam-dark dark:hover:text-pink-400"><input id="file-upload" name="file-upload" type="file" className="sr-only" multiple onChange={handleFileChange} ref={fileInputRef}/><span>Upload temporary files</span></label></div>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2 border border-jam-border dark:border-slate-600 rounded-md p-2">
                    {projectFiles.length > 0 ? projectFiles.map(file => (
                        <div key={file.id} className="flex items-center p-2 bg-slate-100 dark:bg-slate-900/50 rounded-md">
                           <input type="checkbox" id={`proj-file-${file.id}`} checked={selectedProjectFileIds.has(file.id)} onChange={() => handleToggleProjectFile(file.id)} className="w-4 h-4 text-pink-600 bg-slate-100 border-slate-300 rounded focus:ring-pink-500 mr-3"/>
                           <label htmlFor={`proj-file-${file.id}`} className="flex items-center space-x-3 truncate cursor-pointer flex-grow">
            ...
            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                ...
                {tempResources.map(resource => (
                    <div key={resource.id} className="flex items-center justify-between p-2 bg-slate-100 dark:bg-slate-900/50 rounded-md">
                       <div className="flex items-center space-x-3 truncate"><span className="text-xl">{getFileIcon(resource.file.type)}</span><div className="truncate"><p className="text-sm font-medium text-jam-dark dark:text-slate-200 truncate" title={resource.file.name}>{resource.file.name}</p><p className="text-xs text-slate-500 dark:text-slate-400">{formatBytes(resource.file.size)}</p></div></div>
                       <div className="flex items-center space-x-2"><select value={resource.type} onChange={e => updateTempResourceType(resource.id, e.target.value as ResourceType)} className="bg-white dark:bg-slate-800 border border-jam-border dark:border-slate-600 rounded-md py-1 px-2 text-xs text-jam-dark dark:text-slate-200 focus:ring-jam-blue dark:focus:ring-pink-500"><option value="Learning Material">Learning Material</option><option value="Course Exercises">Course Exercises</option><option value="Old Test">Old Test</option></select><button onClick={() => removeTempResource(resource.id)} className="text-slate-400 hover:text-red-500"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clipRule="evenodd"></path></svg></button></div>
                    </div>
                ))}
            </div>
        )}
        ...
        <div className="flex justify-end pt-2">
            <button onClick={handleGenerate} disabled={isLoading} className="w-full flex justify-center items-center px-4 py-2 text-sm font-medium text-white bg-jam-dark rounded-lg hover:bg-black dark:bg-pink-600 dark:hover:bg-pink-700 disabled:bg-slate-400 dark:disabled:bg-slate-600 disabled:cursor-not-allowed">
                {isLoading ? (<><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></div>Generating Plan...</>) : 'Generate Study Sprint'}
            </button>
        </div>
      </div>
    </Modal>
